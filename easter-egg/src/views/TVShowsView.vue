...
const uniqueCategories = computed(() => store.classifications.tv)

const filteredTVShows = computed(() => {
  if (activeCategory.value === 'all') return tvList.value
...
onMounted(async () => {
  // 使用store统一获取数据
  await store.fetchTV()
  await store.fetchClassifications('tv')
})
</script>
